...
J'ai appris à utiliser les hooks `useMemo()` et `useCallback()` qui permettent
de mémoiser des valeurs / fonctions afin d'éviter d'effectuer de nouveaux
calculs / fonctions à chaque nouveau rendu.

Chaque hook prend 2 paramètres :

- Valeur ou callback
- Dépendances

```js
const valueMemo = React.useMemo(() => 1, [])
const cbMemo = React.useCallback(() => fetch(url), [url])
```

Bien que ces hooks permettent un gain de performance, il ne faut pas englober
toutes les valeurs / fonctions de notre application. Cela serait même une perte
de performance car chaque exécution de hook a un coût. Cela complexifie
également la lisibilité de notre code.

Toutefois, si l'on doit par exemple exécuter une fonction au sein d'un effet
`useEffect()`, il faudra encapsuler cette fonction avec le hook `useCallback`
pour éviter qu'elle ne soit re-définie à chaque rendu et déclencher une boucle
infinie

```js
function Component(url) {
  const cbNoMemo = () => fetch(url)
  const cbMemo = React.useCallback(() => fetch(url), [url])

  React.useEffect(() => {
    cbNoMemo() // <-- Boucle infinie
    cbMemo() // <-- Fonction mémoisée
  }, [cbNoMemo, cbMemo])
}
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=02%3A%20useCallback%3A%20custom%20hooks&em=hdessomme%40gmail.com).
