...
J'ai appris à utiliser le hook `useLayoutEffect` qui est similaire au hook
`useEffect` (même API), mais est exécuté avant que React est effectué un rendu
dans le navigateur. 99% du temps, on utilisera `useEffect`, mais dans les cas où
on fera une manipulation du DOM qui aura un impact visuel pour l'utilisateur, il
faudra placer ce code dans le hook `useLayoutEffect`.

[En savoir plus](https://kentcdodds.com/blog/useeffect-vs-uselayouteffect)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=04%3A%20useLayoutEffect%3A%20auto-scrolling%20textarea&em=hdessomme%40gmail.com).
